...
class Surface extends JPanel {
    ...
    public void paintComponent(Graphics g) {
        ...
        if (imageFlag) {
            try {
                ImageIO.write(bufferedImage, "png", new File("C:/result"
                        + indexImage++ + ".png"));
                imageFlag = false;
        ...
        int first = max(1366 - BasicEx.nx, 0);
...
public class BasicEx extends JFrame {
    ...
    public static int nx = 1024;//limit=15M/1gb одной компоненты. ПОВЫСИЛ лимит памяти, и уже Флоат, но 2 компоненты
    public static int ny = 500;//2048*2000(y) = 104sec 2компоненты
    public static double[][] forFurier = new double[2][1];

    public static FirstWindow fW;
